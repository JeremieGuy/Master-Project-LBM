...
if savefiles : 
    new_dir_monitoring = "./Monitoring/Simple_tube_square_obstacle_" + str(maxIter) + "_it"
...
monitorFile = open(new_dir_monitoring + "/monitor_obstacle_node_maxiter=" + str(maxIter) + ".txt", 'w')
nodeX, nodeY = 130, 50
...
plt.title("Flags")
values = unique(flags.ravel())
im = plt.imshow(flags.transpose())
colors = [ im.cmap(im.norm(value)) for value in values]
patches = [ mpatches.Patch(color=colors[i], label="Flag = " + str(int(values[i])) ) for i in range(len(values)) ]
