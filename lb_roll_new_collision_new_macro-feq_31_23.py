...
nx, ny = 31, 23 # Number of lattice nodes.
...
if savefiles : 
    new_dir_monitoring = "./Monitoring/u_roll_31_23_new_collision_" + str(maxIter)
...
flags_plot[:,ny-1] = 1
flags_plot[nx-1,:] = 1
flags_plot[0:24,8:15] = 1

# inlet = 2 
flags_plot[0,1:8] = 2

# outlet = 3
flags_plot[0,15:22] = 3
...
for execTime in range(maxIter):
    ...
    fin[col3,0,15:22] = fin[col3,1,15:22]
    ...
    u[:,0,1:8] = vel[:,0,1:8]
    rho[0,1:8] = 1/(1-u[0,0,1:8]) * ( sum(fin[col2,0,1:8], axis=0) + 2*sum(fin[col3,0,1:8], axis=0) )

    # Compute equilibrium.
    feq = equilibrium(rho, u)
    fin[[0,1,2],0,1:8] = feq[[0,1,2],0,1:8] + fin[[8,7,6],0,1:8] - feq[[8,7,6],0,1:8]
