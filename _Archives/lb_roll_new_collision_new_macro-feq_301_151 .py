...
maxIter = 60000  # Total number of time iterations.
...
def plotSystem2():
    ...
    plt.plot(line1_1,line1_2,color="red", linewidth=1)
    plt.plot(line2_1,line2_2,color="red", linewidth=1)
    plt.plot(line3_1,line3_2,color="red", linewidth=1)
    plt.imshow(flags_plot.transpose())
    # colors = [ im.cmap(im.norm(value)) for value in values]
    # patches = [ mpatches.Patch(color=colors[i], label=flagsname[i] ) for i in range(len(values)) ]
    plt.savefig(new_dir_monitoring + "/system2.png",bbox_inches='tight')
    plt.show()
    plt.close()
