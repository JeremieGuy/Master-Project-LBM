...
###### Flow definition #########################################################
maxIter = 3000  # Total number of time iterations.
...
if savefiles : 
    new_dir_monitoring = "./Monitoring/Monitoring_tube_no_roll_flags_branch_topR_bb_" + str(maxIter) + "_it"
...
if velocityEvolution:
    new_dir_velocity = "./Velocity_Profile/VelocityProfile_flags_branch_topR_bb_" + str(maxIter) + "_it"
...
if saveVisual :
    new_dir_visual = "./Flow/Flow_no_roll_flags_branch_topR_bb_" + str(maxIter) + "_it"
...
for execTime in range(maxIter):
    ...
    # Bottom left: outflow condition.
    # if StopInOut:
    #     if execTime<=(maxIter//2):
    #         fin[col3,-1,:] = fin[col3,-2,:] 
    # else : 
    #     fin[col3,-1,:] = fin[col3,-2,:]
    fin[col1,0,151:ny-2] = fin[col1,1,151:ny-2]
    ...
    u[:,1,1:50] = vel[:,1,1:50]
    rho[1,1:50] = 1/(1-u[0,1,1:50]) * ( sum(fin[col2,1,1:50], axis=0) +
                                2*sum(fin[col3,1,1:50], axis=0) )
       
    # Compute equilibrium.
    feq = equilibrium(rho, u)
    fin[[0,1,2],1,1:50] = feq[[0,1,2],1,1:50] + fin[[8,7,6],1,1:50] - feq[[8,7,6],1,1:50]
