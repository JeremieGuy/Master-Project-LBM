...
for time in range(maxIter):
    ...
    fin[col3,0,151:201] = fin[col3,1,151:201] 

    # bounce back condition
    boundaryFObs = fin[:,obstacle]
    boundaryFTop = fin[:,:,0]
    boundaryFBot = fin[:,:,ny-1]
    boundaryFRight = fin[:,nx-1,1:ny-1]
    
    boundaryFObs = boundaryFObs[[8,7,6,5,4,3,2,1,0],:]
    # top
    boundaryFTop = boundaryFTop[[8,7,6,5,4,3,2,1,0],:]
    # bottom
    boundaryFBot = boundaryFBot[[8,7,6,5,4,3,2,1,0],:]
    # right
    boundaryFRight = boundaryFRight[[8,7,6,5,4,3,2,1,0],:]
    ...
    # Bounce-back apply.
    fout[:,obstacle] = boundaryFObs
    fout[:,:,0] = boundaryFTop
    fout[:,:,ny-1] = boundaryFBot
    fout[:,nx-1,1:ny-1] = boundaryFRight
